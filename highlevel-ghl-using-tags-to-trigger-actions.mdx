...
- The workflow triggers on **Sendspark: New Video Generated - Specific Campaign**, then searches for the contact by email or phone number.
...
Because sometimes the contact already exists and sometimes it doesn’t, we use the **Error handler** entry in a step's overflow menu:
...
| Step | Action | Purpose |
| --- | --- | --- |
| 1 | Trigger: Sendspark: New Video Generated - Specific Campaign | Start the workflow when a video is generated |
| 2 | HighLevel: Search Contact by email or phone number | Check whether the contact already exists |
| 3 | HighLevel: Update a Contact | Apply the tag to the existing contact (uses the Contact ID from Step 2) |
| 4 | Tool: Break | Stop here if the contact was found and updated, preventing a duplicate |
| 5 | Create Contact (via the Error handler branch) | Runs only if no contact was found — create the contact and apply the tag |

## Step 1: Trigger

Start with the trigger: **Sendspark: New Video Generated - Specific Campaign**.

## Step 2: Search for Contact

Add the action **HighLevel: Search Contact by email or phone number**.

- This checks if the contact already exists in GoHighLevel.

## Step 3: Update Contact

Add the action **HighLevel: Update a Contact**.
...
![HighLevel Update Contact action panel with Contact ID field and tag application settings](/images/12161614/384455b8-image.png)

![The field mapping panel, with a tag applied to the contact](/images/12161614/0cbd3bc9-image.png)
...
## Step 5: Create Contact (via the Error handler branch)
...
![The Error handler dialog, offering to stop the automation or move on to a chosen step](/images/12161614/d3b0bdfe-image.png)
...
Once your contacts are tagged when a video is created, you can [automatically generate and send the video via SMS](/ghl-auto-generate-and-send-videos-via-sms).
...
<AccordionGroup>
  ...
  <Accordion title="How do I apply a tag when a Sendspark video is created?">
    Build an Agentic Workflow that triggers on Sendspark: New Video Generated - Specific Campaign, searches for the contact by email or phone, and then updates and tags the contact (if they exist) or creates and tags them (if they don't).
  </Accordion>

...
<Accordion title="Why is there sometimes only one segment?">
  Segments come from the transcription provider, and some return the entire video as a single block. If you need finer divisions, use the subtitle formats below — Sendspark builds those cues from the individual word timings rather than the provider's blocks, so they stay short whatever the provider returned.
</Accordion>
...
| Code | Meaning |
| --- | --- |
| `400` | `format` was something other than `json`, `vtt`, or `srt`. |
